...
module.exports.parseUPSStatus = (body, estimatePower = false, wattNominal = null) => {
  ...
  result.values = {
    ...
    measure_battery_runtime: filled(body['battery.runtime'])
      ? Math.round(parseInt(body['battery.runtime'], 10) / 60)
      : null,
    ...
    measure_voltage: {
      ...
      battery: filled(body['battery.voltage']) ? parseFloat(body['battery.voltage']) : null,
    },
  ...
  if (estimatePower === true) {
    ...
    if (Number.isFinite(result.values.measure_power) && Number.isFinite(vaForPF) && vaForPF > 0) {
      result.values.measure_power_factor = +(result.values.measure_power / vaForPF).toFixed(2);
    }
  }

  if (filled(result.values.status)) {
    let readableStatus = '';
    const statusWords = result.values.status.split(' ');
    result.values.alarm_status = statusWords.includes('OB') || statusWords.includes('LB');

    // eslint-disable-next-line no-return-assign
    statusWords.forEach((word) => readableStatus += `${STATE_TYPES[word]}, `);
