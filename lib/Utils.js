...
module.exports.parseUPSStatus = (body, wattNominal=null) => {
  const STATE_TYPES = {
    OL: 'Online',
    OB: 'On Battery',
    LB: 'Low Battery',
  ...
  if(wattNominal !== null){
    results.measure_power = filled(body['ups.load']) && wattNominal
        ? Math.round((parseInt(body['ups.load'], 10) / 100) * wattNominal)
        : null,
    ...
    if (Number.isFinite(wOut) && Number.isFinite(vaForPF) && vaForPF > 0) {
      ...
      result.values.measure_power_factor = null;
    }
  }
